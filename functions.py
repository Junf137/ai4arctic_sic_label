...
def water_edge_metric(outputs, options):
    ...
    # perform mean
    water_edge_accuracy = 1 - torch.mean(
        torch.abs(outputs[options["charts"][0]] - outputs[options["charts"][1]])
...
def water_edge_plot_overlay(output, mask, options):
    ...
    img = np.concatenate(
        (water_chart[charts[0]], water_chart[charts[1]], water_chart[charts[2]]),
        axis=2,
    )
...
def slide_inference(img, net, options, mode):
    ...
    for h_idx in range(h_grids):
        for w_idx in range(w_grids):
            ...
            if crop_height_pad > 0 or crop_width_pad > 0:
                crop_img = torch.nn.functional.pad(crop_img, (0, crop_width_pad, 0, crop_height_pad), mode="constant", value=0)
            ...
            preds_SIC += torch.nn.functional.pad(
                crop_seg_logit["SIC"], (int(x1), int(preds_SIC.shape[3] - x2), int(y1), int(preds_SIC.shape[2] - y2))
            )
            preds_SOD += torch.nn.functional.pad(
                crop_seg_logit["SOD"], (int(x1), int(preds_SOD.shape[3] - x2), int(y1), int(preds_SOD.shape[2] - y2))
            )
            preds_FLOE += torch.nn.functional.pad(
                crop_seg_logit["FLOE"], (int(x1), int(preds_FLOE.shape[3] - x2), int(y1), int(preds_FLOE.shape[2] - y2))
            )
    ...
    return {"SIC": preds_SIC, "SOD": preds_SOD, "FLOE": preds_FLOE}
...
def batched_slide_inference(img, net, options, mode):
    ...
    height_pad = h_crop - h_img if h_img - h_crop < 0 else (h_stride - (h_img - h_crop) % h_stride) % h_stride
    width_pad = w_crop - w_img if w_img - w_crop < 0 else (w_stride - (w_img - w_crop) % w_stride) % w_stride
    if height_pad > 0 or width_pad > 0:
        img = torch.nn.functional.pad(img, (0, width_pad, 0, height_pad), mode="constant", value=0)
...
def class_decider(output, train_options, chart):
    ...
    if train_options["binary_water_classifier"] == False:
        if output.size(3) == 1:
            ...
            output = torch.clamp(output, min=0, max=train_options["n_classes"][chart])
        else:
...
def compute_classwise_f1score(true, pred, charts, num_classes):
    ...
    score = {}
    for chart in charts:
        score[chart] = f1_score(
            target=true[chart], preds=pred[chart], average="none", task="multiclass", num_classes=num_classes[chart]
        )
    return score
...
def get_scheduler(train_options, optimizer):
    if train_options["scheduler"]["type"] == "CosineAnnealingLR":
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=T_max, eta_min=train_options["scheduler"]["lr_min"]
        )
...
def create_weight_map(arr_np: np.ndarray, cfv: int, ksize: int, threshold: float, weights: dict):
    ...
    # create weight map
    weight_map = np.ones_like(arr_np, dtype=arr_np.dtype) * weights["center"]

    # first applying ice_water_edges, then ice_cfv_edges, thus the intersection will be ice_cfv_edges
    weight_map[inner_edges] = weights["inner_edges"]
    weight_map[ice_water_edge] = weights["ice_water_edges"]
    weight_map[ice_cfv_edge] = weights["ice_cfv_edges"]

    # set all cfv values to 0 in weight_map
    weight_map = np.where(arr_np == cfv, weights["invalid"], weight_map).astype(arr_np.dtype)
